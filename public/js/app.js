...
var sflash = function(options) {
    ...
    function getValue(key) {
        return options[key] || defaultValues[key]
    }

    var type = getValue("type");
    var text = getValue("text");
    var timer = getValue("timer");

    function moveElement(position, endPosition, element, removeElement) {
        setTimeout(function() {
            function transition() {
                if (position == endPosition) {
                    ...
                    position > endPosition ? position -= 10 : position += 10;
                    element.style.left = position + "px";
                }
            }
            var interval = setInterval(transition, 5)
        }, 10)
    }

    function animateElement(transition, element, removeElement) {
        ...
        moveElement(position, endPosition, element, removeElement);
    }

    function createNotice() {
        var spanElem = document.createElement("span");
        spanElem.className = "sf_notice_body";
        spanElem.innerHTML = text;

        var divElem = document.createElement("div");
        divElem.className = "sf_notice";
    ...
    function slideIn(element) {
        animateElement("in", element);
    }

    function slideOut(element) {
        setTimeout(function() {
            animateElement("out", element, true)
        }, timer + 60)
    }
