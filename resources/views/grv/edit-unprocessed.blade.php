    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel panel-primary">
                    <div class="panel-heading mainThemeColor">
                        <div class="row">
                            ...
                            <a href="{{url('unprocessed')}}" class="col-sm-2">

                                <button class="btn-sm btn btn-default btn-raised">Back</button></a>
                            <h3 class="text-center col-sm-6">Receive Goods</h3>
                            <form class="col-sm-4" role="form" method="post" action="{{ url('/receive-unprocessed') }}">
                                {{ csrf_field() }}
                                <div class="input-group">
                                    <select style="width: 100%;" class="form-control select inputs" id="select" name="check" required>
                                        @foreach(\App\Grv::where('status','!=',\App\Grv::SERIALIZED_GRV)->where('state',\App\Grv::UNPROCESSED_GRV)->get() as $grv)
                                            <option onclick="reciveGrv()"  value="{{$grv->autoindex_id}}" >{{$grv->OrderNum}}</option>
                                        @endforeach
                                    </select>
                ...
                <div class="col-md-5">
                    <div id="dnote1" class="panel panel-success dnote1">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <table style="width: 100%" >
                                        ...
                                        <tr>
                                            <th class="pull-left">
                                                <h4>ORDER NO: {{$detail->OrderNum}}</h4>
                                            </th>
                                            <th >
                                        ...
                                        <tr>
                                            <th><h4 class="pull-left">Supplier Name: {{$detail->cAccountName}}</h4></th>
                                            <th ><h5 class="pull-right">Date: {{$detail->created_at}}</h5></th>
                                        </tr>
                                        </thead>
                                    </table>
                                </div>
                        ...
                        <div class="panel-body">
                            <table class="table table-hover table-responsive modalTable" border="1" cellspacing="0">
                                ...
                                {{--@foreach($inlines as $inline)--}}
                                    @if($inlines->qty_serialized >=1)
                                <input type="hidden" value="{{$inlines->id}}" name="id" id="sendId">
                                    <tr>
                                    <td>1</td>
                                   <td>{{$inlines->description}}</td>
                                   <td>{{$inlines->qty_serialized}}</td>
                                        <td><?php $serials = \App\Sageitemsserial::select('serial_one')->where('grv_id',$detail->id)->get();?>
                                        @if(count($serials) > 0)
                                            @foreach($serials as $serial)
                                        <?php echo '<br>'.$serial->serial_one;?>
                        ...
                        <div class="panel-footer" style="background-color: #ffffff">
                            <div class="row">
                                ...
                                <div id="issuebutton" class="pull-right">
                                    ...
                                    <button type="submit" class="btn btn-success btn-sm btn-raised print_grv">
...
@section('script')
    <script>
        $(".print_grv").on('click',function () {
            if($(".modalReceiveNote tr").length) {
